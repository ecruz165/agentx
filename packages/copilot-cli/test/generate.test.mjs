...
import { mkdirSync, writeFileSync, readFileSync, existsSync, readlinkSync, readdirSync, rmSync, mkdtempSync } from 'node:fs';
...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), 'agentx-copilot-'));
  setupMockInstalled();
...
describe('generate', () => {
  it('creates copilot-instructions.md with persona description inline', async () => {
    ...
    assert.ok(result.created.includes(instrPath));

    const content = readFileSync(instrPath, 'utf8');
    // Persona description is inline, no "## Persona" header
    ...
    assert.ok(content.includes('See .github/copilot-context/'));
  });

  it('creates copilot-context symlinks with flattened names', async () => {
    const config = createProjectConfig();
    const result = await generate(config, installedPath, projectPath);

    const link1 = join(projectPath, '.github', 'copilot-context', 'spring-boot-error-handling');
    const link2 = join(projectPath, '.github', 'copilot-context', 'spring-boot-security');
  ...
  it('does not touch other .github/ content', async () => {
    // Pre-create a workflows directory
    const workflowsDir = join(projectPath, '.github', 'workflows');
    mkdirSync(workflowsDir, { recursive: true });
    writeFileSync(join(workflowsDir, 'ci.yaml'), 'name: CI\n');

    const config = createProjectConfig();
    await generate(config, installedPath, projectPath);

    // Verify workflows still intact
    assert.ok(existsSync(join(workflowsDir, 'ci.yaml')));
    assert.equal(readFileSync(join(workflowsDir, 'ci.yaml'), 'utf8'), 'name: CI\n');
  });
  ...
  it('is idempotent', async () => {
    ...
    const instrPath = join(projectPath, '.github', 'copilot-instructions.md');
    assert.ok(result2.updated.includes(instrPath));
  });
  ...
  it('generates with empty config', async () => {
    ...
    assert.ok(existsSync(join(projectPath, '.github', 'copilot-instructions.md')));
  });
});
